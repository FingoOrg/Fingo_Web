...
const Profile = () => {
  ...
  const badges = data.data.find((item) => item.path_id === data.activePath)?.badges || [];
      ...
      <div className="divide-y-2 divide-black w-full flex flex-col mt-7">
        ...
        <div className="grid grid-flow-row grid-cols-2 space-x-2 pt-4">
          <article className="flex flex-col items-start p-2 rounded-lg bg-white shadow-lg">
            ...
            <p className="text-2xl font-medium">${data.amounts.savings_total.toLocaleString('en-US')} MXN</p>
          </article>
          <article className="flex flex-col items-start p-2 rounded-lg bg-white shadow-lg">
            <InvestMoneyIcon />
            <p className="text-md font-semibold mt-2 mb-1">Total invested</p>
            <p className="text-2xl font-medium">${data.amounts.investment_total.toLocaleString('en-US')} MXN</p>
          </article>
        </div>
      </div>
