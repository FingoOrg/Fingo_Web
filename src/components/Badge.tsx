...
const Badge = ({
  ...
  badge: { title: string; type: string; description: string };
}) => {
  ...
  return (
    <article className="flex flex-col justify-center aspect-square items-center p-2 rounded-lg bg-white shadow-lg">
      <Popover className="relative">
        <PopoverButton>{badgeIcon(badge.type)}</PopoverButton>
        <PopoverPanel
          anchor="top"
          transition
        ...
        >
          <p className="text-sm text-black font-bold">{badge.title}</p>
          {badge.description}
        </PopoverPanel>
      </Popover>
    </article>
